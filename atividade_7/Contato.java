...
public class Contato {
    ...
    public void quantos_cont(){
        System.out.println("Existem "+ num +" contatos na agenda");
    }
    ...
    public int calcular_idade(){
        ...
        int idade = c.get(Calendar.YEAR) - dataNascimento[2];

        if(c.get(Calendar.MONTH) < dataNascimento[1]){
            return idade - 1;
        }

        if(c.get(Calendar.MONTH) == dataNascimento[1]){
            if(c.get(Calendar.DAY_OF_MONTH) < dataNascimento[0]){
                return idade --;
            }
        }
